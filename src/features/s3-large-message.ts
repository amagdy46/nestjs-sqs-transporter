...
/**
 * Handles large message offloading to S3.
 *
 * When a message exceeds the configured threshold (default 256KB), it's
 * automatically uploaded to S3 and replaced with a pointer. This allows
 * handling messages larger than SQS's 256KB limit.
 *
 * Supports two pointer formats:
 * - `simple`: Clean format `{ __s3pointer: { bucket, key } }`
 * - `aws-extended`: Java AWS Extended Client format for interoperability
 * - `auto`: Uses simple when writing, detects both when reading
 *
 * @example
 * ```typescript
 * const handler = new S3LargeMessageHandler({
 *   enabled: true,
 *   s3Client: new S3Client({ region: 'us-east-1' }),
 *   bucket: 'my-large-messages',
 *   threshold: 256 * 1024, // 256KB
 *   pointerFormat: 'auto',
 * });
 *
 * // Wrap large message (uploads to S3 if needed)
 * const wrapped = await handler.wrapIfLarge(largeJson);
 *
 * // Unwrap pointer (downloads from S3 if it's a pointer)
 * const original = await handler.unwrapIfPointer(wrapped);
 * ```
 */
export class S3LargeMessageHandler {
	...
	/**
	 * Creates a new S3 large message handler.
	 *
	 * @param options - Configuration options for S3 large message handling
	 */
	...
	private parseS3Pointer(message: string): NormalizedPointer | null {
		try {
			...
			if (
				this.pointerFormat === "aws-extended" ||
				this.pointerFormat === "auto"
			) {
